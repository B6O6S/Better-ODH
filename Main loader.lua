...
local runtime = { modules = {}, pending = 0, ready = 0, failed = 0, version = "2026-09-19 modular" }
...
local function contextFor(record)
    local ctx = { section = nil, initializing = true, controls = 0, errors = {} }
    local raw = record.rawSection
    local section = {}
    ctx.section = section
    local function guarded(callback, name)
        return function(...)
            if ctx.initializing then return end -- constructor callbacks must not run half-initialized code
            local ok, result = pcall(callback, ...)
            if not ok then
                local message = tostring(name) .. ": " .. tostring(result)
                if not ctx.errors[message] then
                    ctx.errors[message] = true
                    warn("[Better ODH / " .. record.name .. "] " .. message)
                    notify(record.name .. ": action failed; see console.", 4)
                end
            end
    ...
    for method, position in pairs(callbackPositions) do
        section[method] = function(_, ...)
            ...
            args[position] = guarded(args[position], args[1])
            local result = raw[method](raw, table.unpack(args, 1, args.n))
            ctx.controls = ctx.controls + 1
            return result -- preserve documented control objects/toggle closures
        end
    end
    function section:AddLabel(...)
        local result = raw:AddLabel(...)
        ctx.controls = ctx.controls + 1
        return result
    end
    function section:AddParagraph(...)
        local result = raw:AddParagraph(...)
        ctx.controls = ctx.controls + 1
        return result
    end
    -- Legacy calls are redirected only inside this module. Never modify host.
