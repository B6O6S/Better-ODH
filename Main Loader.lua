-- Consolidated new-API loader (Silent execution mode)
...
local function contextFor(record)
    ...
    shared.Notify=function(text,seconds) end
...
-- Persistent Settings Table for GUI Positions (.json save)
local function getSavedSettings()
    local success, result = pcall(function()
        if readfile and isfile and isfile("ODH_GUI_Positions.json") then
            return HttpService:JSONDecode(readfile("ODH_GUI_Positions.json"))
        end
    end)
...
local function saveSettings(settingsTbl)
    pcall(function()
        if writefile then
            writefile("ODH_GUI_Positions.json", HttpService:JSONEncode(settingsTbl))
        end
    end)
end

local guiPositions = getSavedSettings()

-- Global Theme Color State for Strokes
local currentGuiColor = Color3.fromRGB(255, 255, 255)
local strokesList = {}

local function updateAllStrokes(color)
    currentGuiColor = color
    for _, stroke in ipairs(strokesList) do
        if stroke and stroke.Parent then
            -- Skip if it's currently showing status colors like green (Ready) or red (Wait timer)
            -- We can check standard color or just force apply if needed. Here we apply directly.
            stroke.Color = color
        end
    end
end
...
local globalColorPicker = trickshot_section:AddColorpicker("GUI Border Color", Color3.fromRGB(255, 255, 255), function(color3rgb)
    updateAllStrokes(color3rgb)
end)
...
local jumpBoostGuiSize = 220
local jump360Enabled = false
local jump360Size = 220
local humanoid
local jumpBoostActive = false
local originalJumpPower = 50
local spinOnNextJump = false

-- Jump Boost Draggable GUI (Square 1:1, Lag-Free Release-Only Saving)
local screenGuiJump = CoreGui:FindFirstChild("AutoJumpBoostDraggableGui")
local frameJump, buttonJump, strokeJump
local defaultJumpPos = UDim2.new(1, -240, 1, -90)

if not screenGuiJump then
    screenGuiJump = Instance.new("ScreenGui")
    screenGuiJump.Name = "AutoJumpBoostDraggableGui"
    screenGuiJump.ResetOnSpawn = false
    screenGuiJump.Enabled = false
    screenGuiJump.Parent = CoreGui

    frameJump = Instance.new("Frame")
    frameJump.Size = UDim2.new(0, jumpBoostGuiSize, 0, jumpBoostGuiSize) -- Square 1:1
    frameJump.Position = guiPositions.JumpPos or defaultJumpPos
    frameJump.BackgroundTransparency = 1
    frameJump.Parent = screenGuiJump

    local corner = Instance.new("UICorner")
    corner.CornerRadius = UDim.new(0, 16)
    corner.Parent = frameJump

    strokeJump = Instance.new("UIStroke")
    strokeJump.Color = currentGuiColor
    strokeJump.Thickness = 3.5
    strokeJump.Parent = frameJump
    table.insert(strokesList, strokeJump)

    buttonJump = Instance.new("TextButton")
    buttonJump.Size = UDim2.new(1, 0, 1, 0)
    buttonJump.BackgroundTransparency = 1
    buttonJump.Text = "Jump Boost (OFF)"
    buttonJump.TextColor3 = Color3.fromRGB(255, 255, 255)
    buttonJump.TextSize = math.clamp(math.floor(jumpBoostGuiSize / 12), 10, 36)
    buttonJump.Font = Enum.Font.GothamBold
    buttonJump.Parent = frameJump
else
    frameJump = screenGuiJump:FindFirstChildOfClass("Frame")
    strokeJump = frameJump and frameJump:FindFirstChildOfClass("UIStroke")
    if strokeJump then table.insert(strokesList, strokeJump) end
    buttonJump = frameJump and frameJump:FindFirstChildOfClass("TextButton")
end

local draggingJump, dragInputJump, dragStartJump, startPosJump, movedJump = false, nil, nil, nil, false
if buttonJump and frameJump then
    buttonJump.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
            draggingJump, movedJump = true, false
            dragStartJump, startPosJump = input.Position, frameJump.Position
            ...
            connection = input.Changed:Connect(function()
                if input.UserInputState == Enum.UserInputState.End then
                    ...
                    -- SAVE ONLY ON RELEASE (ZERO LAG WHILE DRAGGING)
                    guiPositions.JumpPos = frameJump.Position
                    saveSettings(guiPositions)
                end
            end)
        else
            movedJump = true
    ...
    buttonJump.InputChanged:Connect(function(input)
    ...
    UIS.InputChanged:Connect(function(input)
        if input == dragInputJump and draggingJump then
            ...
            if math.abs(delta.X) > 5 or math.abs(delta.Y) > 5 then movedJump = true end
            frameJump.Position = UDim2.new(startPosJump.X.Scale, startPosJump.X.Offset + delta.X, startPosJump.Y.Scale, startPosJump.Y.Offset + delta.Y)
        end
    end)

    buttonJump.MouseButton1Click:Connect(function()
        if movedJump then return end
        jumpBoostActive = not jumpBoostActive
        if jumpBoostActive then
            buttonJump.Text = "Ready"
            buttonJump.TextColor3 = Color3.fromRGB(50, 255, 50)
            if strokeJump then strokeJump.Color = Color3.fromRGB(50, 200, 50) end
            if humanoid then
            ...
            buttonJump.Text = "Jump Boost (OFF)"
            buttonJump.TextColor3 = Color3.fromRGB(255, 255, 255)
            if strokeJump then strokeJump.Color = currentGuiColor end
            if humanoid then
...
-- 360 Jump Draggable GUI (Square 1:1, Lag-Free Release-Only Saving)
local screenGui360 = CoreGui:FindFirstChild("Auto360JumpDraggableGui")
local frame360, button360, stroke360
local default360Pos = UDim2.new(0.5, -110, 0.5, -50)

if not screenGui360 then
    screenGui360 = Instance.new("ScreenGui")
    screenGui360.Name = "Auto360JumpDraggableGui"
    screenGui360.ResetOnSpawn = false
    screenGui360.Enabled = false
    screenGui360.Parent = CoreGui

    frame360 = Instance.new("Frame")
    frame360.Size = UDim2.new(0, jump360Size, 0, jump360Size) -- Square 1:1
    frame360.Position = guiPositions.Jump360Pos or default360Pos
    frame360.BackgroundTransparency = 1
    frame360.Parent = screenGui360

    local corner = Instance.new("UICorner")
    corner.CornerRadius = UDim.new(0, 16)
    corner.Parent = frame360

    stroke360 = Instance.new("UIStroke")
    stroke360.Color = currentGuiColor
    stroke360.Thickness = 3.5
    stroke360.Parent = frame360
    table.insert(strokesList, stroke360)

    button360 = Instance.new("TextButton")
    button360.Size = UDim2.new(1, 0, 1, 0)
    button360.BackgroundTransparency = 1
    button360.Text = "360 Jump (OFF)"
    button360.TextColor3 = Color3.fromRGB(255, 255, 255)
    button360.TextSize = math.clamp(math.floor(jump360Size / 12), 10, 36)
    button360.Font = Enum.Font.GothamBold
    button360.Parent = frame360
else
    frame360 = screenGui360:FindFirstChildOfClass("Frame")
    stroke360 = frame360 and frame360:FindFirstChildOfClass("UIStroke")
    if stroke360 then table.insert(strokesList, stroke360) end
    button360 = frame360 and frame360:FindFirstChildOfClass("TextButton")
end

local dragging360, dragInput360, dragStart360, startPos360, moved360 = false, nil, nil, nil, false
if button360 and frame360 then
    button360.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
            dragging360, moved360 = true, false
            dragStart360, startPos360 = input.Position, frame360.Position
            ...
            connection = input.Changed:Connect(function()
                if input.UserInputState == Enum.UserInputState.End then
                    ...
                    if connection then connection:Disconnect() end
                    -- SAVE ONLY ON RELEASE (ZERO LAG WHILE DRAGGING)
                    guiPositions.Jump360Pos = frame360.Position
                    saveSettings(guiPositions)
                end
            end)
        else
    ...
    button360.InputChanged:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseMovement then
            dragInput360 = input
            moved360 = true
        end
    ...
    UIS.InputChanged:Connect(function(input)
        if input == dragInput360 and dragging360 then
            local delta = input.Position - dragStart360
            if math.abs(delta.X) > 5 or math.abs(delta.Y) > 5 then moved360 = true end
            frame360.Position = UDim2.new(startPos360.X.Scale, startPos360.X.Offset + delta.X, startPos360.Y.Scale, startPos360.Y.Offset + delta.Y)
        end
    end)

    button360.MouseButton1Click:Connect(function()
        if moved360 then return end
        spinOnNextJump = not spinOnNextJump
        if spinOnNextJump then
            button360.Text = "Ready"
            button360.TextColor3 = Color3.fromRGB(50, 255, 50)
            if stroke360 then stroke360.Color = Color3.fromRGB(50, 200, 50) end
        else
            button360.Text = "360 Jump (OFF)"
            button360.TextColor3 = Color3.fromRGB(255, 255, 255)
            if stroke360 then stroke360.Color = currentGuiColor end
        end
    end)
end
...
local function setupHumanoid(h)
    ...
    h:GetPropertyChangedSignal("Jump"):Connect(function()
        if jumpBoostActive and h.Jump then
            task.wait(0.1)
        ...
        -- Lag-free single-execution 360 spin on jump
        if jump360Enabled and spinOnNextJump and h.Jump then
            ...
            task.spawn(function()
                ...
                if HRP then
                    local totalSteps = 15
                    local stepAngle = (math.pi * 2) / totalSteps
...
trickshot_section:AddToggle("Enable Jump Boost Button", function(on)
    jumpBoostEnabled = on
    if screenGuiJump then 
        screenGuiJump.Enabled = on 
        if on and frameJump then
            frameJump.Position = guiPositions.JumpPos or defaultJumpPos
        end
    end
    if not on and jumpBoostActive then
        jumpBoostActive = false
        if buttonJump then
            buttonJump.Text = "Jump Boost (OFF)"
            buttonJump.TextColor3 = Color3.fromRGB(255, 255, 255)
...
trickshot_section:AddSlider("Jump Boost GUI Size", 50, 300, jumpBoostGuiSize, function(v)
    jumpBoostGuiSize = v
    if frameJump then
        frameJump.Size = UDim2.new(0, jumpBoostGuiSize, 0, jumpBoostGuiSize)
    end
    if buttonJump then
        buttonJump.TextSize = math.clamp(math.floor(jumpBoostGuiSize / 12), 10, 36)
    end
...
trickshot_section:AddToggle("Enable 360 Jump Button", function(on)
    jump360Enabled = on
    if screenGui360 then 
        screenGui360.Enabled = on 
        if on and frame360 then
            frame360.Position = guiPositions.Jump360Pos or default360Pos
        end
    end
    if not on and spinOnNextJump then
        spinOnNextJump = false
        if button360 then
            button360.Text = "360 Jump (OFF)"
            button360.TextColor3 = Color3.fromRGB(255, 255, 255)
...
trickshot_section:AddSlider("360 Jump Button Size", 50, 300, jump360Size, function(v)
    jump360Size = v
    if frame360 then
        frame360.Size = UDim2.new(0, jump360Size, 0, jump360Size)
    end
    if button360 then
        button360.TextSize = math.clamp(math.floor(jump360Size / 12), 10, 36)
    end
...
local defaultBombPos = UDim2.new(1, -240, 1, -210)
...
if not screenGuiBomb then
    ...
    frameBomb.Position = guiPositions.BombPos or defaultBombPos
    ...
    strokeBomb.Color = currentGuiColor
    strokeBomb.Thickness = 3.5
    strokeBomb.Parent = frameBomb
    table.insert(strokesList, strokeBomb)
    ...
    buttonBomb.TextSize = math.clamp(math.floor(bombJumpGuiSize / 12), 9, 28)
    ...
    if strokeBomb then table.insert(strokesList, strokeBomb) end
        ...
        if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
            ...
            connection = input.Changed:Connect(function()
                if input.UserInputState == Enum.UserInputState.End then
                    ...
                    if connection then connection:Disconnect() end
                    -- SAVE ONLY ON RELEASE (ZERO LAG WHILE DRAGGING)
                    guiPositions.BombPos = frameBomb.Position
                    saveSettings(guiPositions)
                end
            end)
        else
...
local function executeBombJumpAction(isToggleMode)
    ...
    task.spawn(function()
        ...
        buttonBomb.TextColor3, strokeBomb.Color = Color3.fromRGB(255, 255, 255), currentGuiColor
    end)
end
...
if buttonBomb then
    buttonBomb.MouseButton1Click:Connect(function()
        ...
        if bombUseMode == "Toggle To Use Bomb" then
            ...
            strokeBomb.Color = isActiveBomb and Color3.fromRGB(50, 200, 50) or currentGuiColor
...
trickshot_section:AddToggle("Show Bomb Jump Button", function(on)
    showBombJumpGui = on
    if screenGuiBomb then 
        screenGuiBomb.Enabled = on 
        if on and frameBomb then
            frameBomb.Position = guiPositions.BombPos or defaultBombPos
        end
    end
end)
...
trickshot_section:AddSlider("Bomb Jump GUI Size", 50, 300, bombJumpGuiSize, function(v)
    bombJumpGuiSize = v
    if frameBomb then 
        frameBomb.Size = UDim2.new(0, bombJumpGuiSize, 0, bombJumpGuiSize * (100/220)) 
    end
...
local defaultFlickPos = UDim2.new(1, -240, 1, -330)
...
if not flickGui then
    ...
    flickFrame.Position = guiPositions.FlickPos or defaultFlickPos
    ...
    flickStroke.Color, flickStroke.Thickness, flickStroke.Transparency = currentGuiColor, 3.5, 0
    table.insert(strokesList, flickStroke)
    ...
    flickButton.Text, flickButton.TextColor3, flickButton.TextSize, flickButton.Font = "Flick Shoot", Color3.fromRGB(255, 255, 255), math.clamp(math.floor(flickGuiSize / 12), 9, 28), Enum.Font.GothamBold
else
    ...
    if flickFrame then
        ...
        if flickStroke then table.insert(strokesList, flickStroke) end
...
if flickButton and flickFrame then
        ...
        if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
            ...
            connection = input.Changed:Connect(function()
                if input.UserInputState == Enum.UserInputState.End then
                    ...
                    if connection then connection:Disconnect() end
                    -- SAVE ONLY ON RELEASE (ZERO LAG WHILE DRAGGING)
                    guiPositions.FlickPos = flickFrame.Position
                    saveSettings(guiPositions)
                end
            end)
        end
    ...
    local normalTextColor, reloadColor = Color3.fromRGB(255, 255, 255), Color3.fromRGB(255, 0, 0)
    ...
    local function setNormalVisuals()
        ...
        if flickStroke then flickStroke.Color = currentGuiColor end
    end
    ...
    trickshot_section:AddSlider("Flick Shoot GUI Size", 50, 300, flickGuiSize, function(v)
        flickGuiSize = v
        if flickFrame then 
            flickFrame.Size = UDim2.new(0, flickGuiSize, 0, flickGuiSize * (100 / 220)) 
        end
        if flickButton then
            flickButton.TextSize = math.clamp(math.floor(flickGuiSize / 12), 9, 28)
        end
    end)
end
...
-- Draggable Logic with Click Blocking
local dragging, moved, dragInput, dragStart, startPos = false, false, nil, nil, nil
    ...
    if active then
        ...
        if rootPart and humanoid then
            ...
            connection = RunService.RenderStepped:Connect(function()
                ...
                -- Check for sudden teleports / server moves to prevent desync bugs
                local currentPos = rootPart.Position
                if lastRootPos and (currentPos - lastRootPos).Magnitude > 25 then
...
-- Reset state safely on character respawn or teleport
...
function createToggleButton()
    ...
    -- Custom Draggable & Click-Blocking State
    ...
    toggleButton.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
            dragging, moved = true, false
            dragStart, startPos = input.Position, toggleButton.Position
...
-- Function to lock graphics to Level 1 for 3 seconds to prevent lag/max overrides
local function enforceLowGraphicsFor3Seconds()
    task.spawn(function()
        local startTime = tick()
...
-- Run it immediately upon script initialization/section load
enforceLowGraphicsFor3Seconds()
...
local function applyRTXPreset()
    ...
    -- Prevent RTX preset from triggering high quality level override
    enforceLowGraphicsFor3Seconds()
...
-- Custom Graphics Sliders (Guarded)
...
local spray_section = spray_tab:AddSection("SprayPaint Mods", "MM2/MMV")
...
local function getGameTextBox()
    ...
    if not pg then return nil end
    return pg:FindFirstChild("SprayGui")
        and pg.SprayGui:FindFirstChild("Main")
        and pg.SprayGui.Main:FindFirstChild("Container")
        and pg.SprayGui.Main.Container:FindFirstChild("Add")
        and pg.SprayGui.Main.Container.Add:FindFirstChild("Frame")
        and pg.SprayGui.Main.Container.Add.Frame:FindFirstChild("Container")
        and pg.SprayGui.Main.Container.Add.Frame.Container:FindFirstChild("TextBox")
end
...
local function isImageLoadedInGui(assetId)
    ...
    local pg = player:FindFirstChildOfClass("PlayerGui") or player
    local searchRoot = pg:FindFirstChild("SprayGui") or pg
...
-- Unlock and allow immediate re-input on character respawn
player.CharacterAdded:Connect(function()
    isSeqRunning = false
    task.wait(1.5)
end)

-- Continuous Enforcer / Monitor Loop
task.spawn(function()
...
-- Create all sections on ONE tab, in a deterministic order without loading labels.
for _, definition in ipairs(definitions) do
    local record = { name = definition.name, state = "Queued", controls = 0 }
    runtime.modules[#runtime.modules + 1] = record
    local ok, section = pcall(function() return rootTab:AddSection(definition.name, "MM2/MMV") end)
...
for _, record in ipairs(runtime.modules) do
    if record.context then
        -- Run initialization asynchronously on a low-priority thread to prevent frame hitching
        task.defer(function()
